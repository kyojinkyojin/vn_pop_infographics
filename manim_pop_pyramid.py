...
class PopulationPyramid(Scene):
    def construct(self):
        ...
        male_bars = VGroup(*[
            Rectangle(
                width=pop, height=0.5, fill_color=BLUE, fill_opacity=0.8,
                stroke_color=BLACK, stroke_width=1
            ).shift(LEFT * pop/2+ DOWN * i * 0.6)
        ...
        female_bars = VGroup(*[
            Rectangle(
                width=pop, height=0.5, fill_color=PINK, fill_opacity=0.8,
                stroke_color=BLACK, stroke_width=1
            ).shift(RIGHT * pop/2 + DOWN * i * 0.6)
            for i, pop in enumerate(female_population)
        ])

        # Create age labels
        age_labels_l = VGroup(*[
            Text(age, font_size=10).next_to(male_bars[i], LEFT, buff=0.1)
            for i, age in enumerate(age_groups)
        ])
        
        age_labels_r = VGroup(*[
            Text(age, font_size=10).next_to(female_bars[i], RIGHT, buff=0.1)
            for i, age in enumerate(age_groups)
        ])

        # Create population labels for males
        male_labels = VGroup(*[
            Text(f"{'{0:.2f}'.format(pop)}%", font_size=24).next_to(male_bars[i], RIGHT, buff=0.1)
            for i, pop in enumerate(male_population)
        ])

        # Create population labels for females
        female_labels = VGroup(*[
            Text(f"{'{0:.2f}'.format(pop)}%", font_size=24).next_to(female_bars[i], LEFT, buff=0.1)
            for i, pop in enumerate(female_population)
        ])
